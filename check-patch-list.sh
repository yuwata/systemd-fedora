...
if ! git -C $REPO_DIR checkout master; then
    echo 'error: cannot checkout master'
    exit 201
fi

if ! git -C $REPO_DIR pull upstream master; then
    echo 'error: cannot pull upstream master' >&2
    exit 202
fi
...
for patch in $PATCHES; do
    if ! git -C $REPO_DIR am ${SOURCE_DIR}/$patch; then
        ...
        git -C $REPO_DIR checkout master && git -C $REPO_DIR branch -D $TEST_BRANCH
        exit 204
    fi
done
