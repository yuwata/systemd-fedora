...
for branch in $BRANCHES; do
    PATCH_LIST=$(git -C $REPO_DIR format-patch -o ${SOURCE_DIR} --no-numbered --no-signature --start-number ${start} $(git -C $REPO_DIR show-branch --merge-base refs/remotes/upstream/master ${branch})..${branch})
